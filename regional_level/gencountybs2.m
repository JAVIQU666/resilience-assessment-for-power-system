...
for ppp=1:4
    ...
    countyresultbs = cell(size(monthly_average_load,1),3);
    countyresultbsinfo = cell(size(monthly_average_load,1),32);
    ...
    for i=1:numcounty
        ...
        for stage = 1:numstage
            ...
            if CtgProbheat ~= 0
                ...
                for j = 1:numScenarios
                    scenarioData = eval([scenarioNames{j}, '(10, indicespv)']);
                    leng = size(scenarioData, 2);
            ...
            if CtgProbtyphoon ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(10, indicestyphoon1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(10, indicestyphoon2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(10, indicestyphoon3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbexwind~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(10, indicesexwind1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(10, indicesexwind2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(10, indicesexwind3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbflood ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(10, indicesflood1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(10, indicesflood2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(10, indicesflood3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            rigidcosttotal(stage) = 0.9*rant(stage)/numnetwork*(max([rigidcostnormal(stage),rigidcostheat(stage),rigidcostcold(stage),rigidcosttyphoon(stage),rigidcostexwind(stage),rigidcostflood(stage)]));
            ...
            temp1 = [rant(stage)*relapercost/70*numnetwork*[relanormal(stage),relaheat(stage),relacold(stage),0.3*relatyphoon(stage),0.3*relaexwind(stage),relaflood(stage)],
                ...
                0.9*rant(stage)/discount*numnetwork*[rigidcostnormal(stage),rigidcostheat(stage),rigidcostcold(stage),rigidcosttyphoon(stage),rigidcostexwind(stage),rigidcostflood(stage)],
        ...
        countyresultbs{i,1} = monthly_average_load(i,1);
        countyresultbs{i,2} = found_row;
        countyresultbs{i,3} = result;

        weather_conditions = {'normal', 'heat', 'cold', 'typhoon', 'exwind', 'flood'};
        countyresultbsinfo{i,1} = monthly_average_load(i,1);
        countyresultbsinfo{i,2} = found_row;
        for j =1:length(temptotal)
            countyresultbsinfo{i,j+2} = temptotal(j);
        end

        resultrela.temprela = temprela;
        resultrelapower.temprela = temprelapower;
        countyrelabs{i,1} = monthly_average_load(i,1);
        countyrelabs{i,2} = found_row;
        countyrelabs{i,3} = resultrela;
        countyrelabspower{i,1} = monthly_average_load(i,1);
        countyrelabspower{i,2} = found_row;
        countyrelabspower{i,3} = resultrelapower;
        i;
    end

    chinaresultbs = [Tgencostnormal;Trelanormal;TDRcostnormal;Trigidcostnormal;Tdistributecostnormal;Tbscostnormal];
    chinaresultbs = [chinaresultbs,sum(chinaresultbs,2)];
    chinaresultbs(4:end,7) = chinaresultbs(4:end,6);
    if ppp == 1
        save('countyresultoptinfo126.mat','countyresultbsinfo');
        save('countyresultbs126.mat','countyresultbs');
        save('chinaresultbs126.mat','chinaresultbs');
        save('countyrelabs126.mat','countyrelabs');%without bulk power system
        save('countyrelabspower126.mat','countyrelabspower');
    elseif ppp == 2
        save('countyresultoptinfo245.mat','countyresultbsinfo');
        save('countyresultbs245.mat','countyresultbs');
        save('chinaresultbs245.mat','chinaresultbs');
        save('countyrelabs245.mat','countyrelabs');
        save('countyrelabspower245.mat','countyrelabspower');
    elseif ppp == 3
        save('countyresultoptinfo370.mat','countyresultbsinfo');
        save('countyresultbs370.mat','countyresultbs');
        save('chinaresultbs370.mat','chinaresultbs');
        save('countyrelabs370.mat','countyrelabs');
        save('countyrelabspower370.mat','countyrelabspower');
    elseif ppp == 4
        save('countyresultoptinfo585.mat','countyresultbsinfo');
        save('countyresultbs585.mat','countyresultbs');
        save('chinaresultbs585.mat','chinaresultbs');
        save('countyrelabs585.mat','countyrelabs');
        save('countyrelabspower585.mat','countyrelabspower');
    end
end