...
for ppp=1:4
    ...
    countyresultmicro = cell(size(monthly_average_load,1),3);
    ...
    for i=1:numcounty
        ...
        for stage = 1:numstage
            ...
            if CtgProbheat ~= 0
                ...
                for j = 1:numScenarios
                    scenarioData = eval([scenarioNames{j}, '(11, indicespv)']);
                    leng = size(scenarioData, 2);
            ...
            if CtgProbtyphoon ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(11, indicestyphoon1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(11, indicestyphoon2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(11, indicestyphoon3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbexwind~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(11, indicesexwind1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(11, indicesexwind2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(11, indicesexwind3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbflood ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(11, indicesflood1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(11, indicesflood2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(11, indicesflood3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
        ...
        countyresultmicro{i,1} = monthly_average_load(i,1);
        countyresultmicro{i,2} = found_row;
        countyresultmicro{i,3} = result;

        weather_conditions = {'normal', 'heat', 'cold', 'typhoon', 'exwind', 'flood'};
        countyresultmicroinfo{i,1} = monthly_average_load(i,1);
        countyresultmicroinfo{i,2} = found_row;
        for j =1:length(temptotal)
            countyresultmicroinfo{i,j+2} = temptotal(j);
        end
        resultrela.temprela = temprela;
        resultrelapower.temprela = temprelapower;
        countyrelamicro{i,1} = monthly_average_load(i,1);
        countyrelamicro{i,2} = found_row;
        countyrelamicro{i,3} = resultrela;
        countyrelamicropower{i,1} = monthly_average_load(i,1);
        countyrelamicropower{i,2} = found_row;
        countyrelamicropower{i,3} = resultrelapower;
        i;
    end
    chinaresultmicro = [Tgencostnormal;Trelanormal;TDRcostnormal;Trigidcostnormal;Tdistributecostnormal;Tbscostnormal];
    chinaresultmicro = [chinaresultmicro,sum(chinaresultmicro,2)];
    chinaresultmicro(4:end,7) = chinaresultmicro(4:end,6);
    if ppp == 1
        save('countyresultmicro126.mat','countyresultmicro');
        save('chinaresultmicro126.mat','chinaresultmicro');
        save('countyrelamicro126.mat','countyrelamicro');
        save('countyrelamicropower126.mat','countyrelamicropower');
    elseif ppp == 2
        save('countyresultmicro245.mat','countyresultmicro');
        save('chinaresultmicro245.mat','chinaresultmicro');
        save('countyrelamicro245.mat','countyrelamicro');
        save('countyrelamicropower245.mat','countyrelamicropower');
    elseif ppp == 3
        save('countyresultmicro370.mat','countyresultmicro');
        save('chinaresultmicro370.mat','chinaresultmicro');
        save('countyrelamicro370.mat','countyrelamicro');
        save('countyrelamicropower370.mat','countyrelamicropower');
    elseif ppp == 4
        save('countyresultmicro585.mat','countyresultmicro');
        save('chinaresultmicro585.mat','chinaresultmicro');
        save('countyrelamicro585.mat','countyrelamicro');
        save('countyrelamicropower585.mat','countyrelamicropower');
    end
end