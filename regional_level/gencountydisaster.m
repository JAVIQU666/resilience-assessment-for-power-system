...
for ppp=1:4
    ...
    countyresultdisaster = cell(size(monthly_average_load,1),3);
    ...
    for i=1:numcounty
        ...
        weightsnetwork = ceil(weightsnetwork')*indexadjest_avg(found_row); 
        %% pinlv
        ...
        for stage = 1:numstage
            ...
            %% duration
            %res1 = precalc(monthly_average_load(i,1), 1);
            res = precalc(monthly_average_load(i,1), stage);
            % res = precalc(monthly_average_load(i,1), stage);
            % 概率
            %         CtgProbcold1    = res1.CtgProbcold/365;
            %         CtgProbexwind1  = res1.CtgProbexwind/365;
            %         CtgProbheat1    = res1.CtgProbheat/365;
            %         CtgProbtyphoon1 = res1.CtgProbtyphoon/365;
            %         CtgProbnormal1  = res1.CtgProbnormal;
            %         CtgProbflood1   = res1.CtgProbflood;
            ...
            %
            %         weightsloadheatstage    = res.heat_weight;
            %         weightsloadcoldstage    = res.cold_weight;
            %         weightsloadtyphoonstage = res.typhoon_weight;
            %         weightsloadexwindstage  = res.exwind_weight;
            %         weightsloadfloodstage   = res.fl_weight;
            ...
            if CtgProbcold~=0
                ...
                if found_row ~= 2 || found_row ~= 9 || found_row ~= 20 || found_row ~= 25 || found_row ~= 28 || found_row ~= 12 || found_row ~= 15 || found_row ~= 18
                    ...
                    relacold(stage) = relacold(stage) + CtgProbcold*weightsnetwork*cell2mat(results(:, 2));
            ...
            if CtgProbtyphoon ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(9, indicestyphoon1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(9, indicestyphoon2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(9, indicestyphoon3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbexwind~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(9, indicesexwind1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(9, indicesexwind2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(9, indicesexwind3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbflood ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(9, indicesflood1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(9, indicesflood2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(9, indicesflood3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            DRcosttotal(stage) = rant(stage)/discount*numnetwork*(DRcostnormal(stage)+DRcostheat(stage)+DRcostcold(stage)+DRcosttyphoon(stage)+DRcostexwind(stage)+DRcostflood(stage));
            rigidcosttotal(stage) = 0.9*rant(stage)/discount*numnetwork*(max([rigidcostnormal(stage),rigidcostheat(stage),rigidcostcold(stage),rigidcosttyphoon(stage),rigidcostexwind(stage),rigidcostflood(stage)]));
            distributecosttotal(stage) = huilv*rant(stage)*numnetwork*(max([distributecostbsnormal(stage),distributecostbsheat(stage),distributecostbscold(stage),distributecostbstyphoon(stage),distributecostbsexwind(stage),distributecostbsflood(stage)])+...
                max([distributecostpvnormal(stage),distributecostpvheat(stage),distributecostpvcold(stage),distributecostpvtyphoon(stage),distributecostpvexwind(stage),distributecostpvflood(stage)]));
            ...
            temp1 = [rant(stage)*relapercost/70*numnetwork*[relanormal(stage),relaheat(stage),relacold(stage),0.3*relatyphoon(stage),0.3*relaexwind(stage),relaflood(stage)],
                rant(stage)/discount*numnetwork*[DRcostnormal(stage),DRcostheat(stage),DRcostcold(stage),DRcosttyphoon(stage),DRcostexwind(stage),DRcostflood(stage)],
                0.9*rant(stage)*numnetwork*[rigidcostnormal(stage),rigidcostheat(stage),rigidcostcold(stage),rigidcosttyphoon(stage),rigidcostexwind(stage),rigidcostflood(stage)],
                huilv*rant(stage)*numnetwork*[distributecostbsnormal(stage)+distributecostpvnormal(stage),distributecostbsheat(stage)+distributecostpvheat(stage),distributecostbscold(stage)+distributecostpvcold(stage),distributecostbstyphoon(stage)+distributecostpvtyphoon(stage),distributecostbsexwind(stage)+distributecostpvexwind(stage),distributecostbsflood(stage)+distributecostpvflood(stage)],
                huilv*rant(stage)*numnetwork*[bscostnormal(stage),bscostheat(stage),bscostcold(stage),bscosttyphoon(stage),bscostexwind(stage),bscostflood(stage)]];
        ...
        countyresultdisaster{i,1} = monthly_average_load(i,1);
        countyresultdisaster{i,2} = found_row;
        countyresultdisaster{i,3} = result;

        weather_conditions = {'normal', 'heat', 'cold', 'typhoon', 'exwind', 'flood'};
        countyresultdisasterinfo{i,1} = monthly_average_load(i,1);
        countyresultdisasterinfo{i,2} = found_row;
        for j =1:length(temptotal)
            countyresultdisasterinfo{i,j+2} = temptotal(j);
        end
        resultrela.temprela = temprela;
        resultrelapower.temprela = temprelapower;
        countyreladisaster{i,1} = monthly_average_load(i,1);
        countyreladisaster{i,2} = found_row;
        countyreladisaster{i,3} = resultrela;
        countyreladisasterpower{i,1} = monthly_average_load(i,1);
        countyreladisasterpower{i,2} = found_row;
        countyreladisasterpower{i,3} = resultrelapower;
        i
    end
    chinaresultdisaster = [Tgencostnormal;Trelanormal;TDRcostnormal;Trigidcostnormal;Tdistributecostnormal;Tbscostnormal];
    chinaresultdisaster = [chinaresultdisaster,sum(chinaresultdisaster,2)];
    if ppp == 1
        save('countyresultdisasterinfo126.mat','countyresultdisasterinfo');
        save('countyresultdisaster126.mat','countyresultdisaster');
        save('chinaresultdisaster126.mat','chinaresultdisaster');
        save('countyreladisaster126.mat','countyreladisaster');
        save('countyreladisasterpower126.mat','countyreladisasterpower');
    elseif ppp == 2
        save('countyresultdisasterinfo245.mat','countyresultdisasterinfo');
        save('countyresultdisaster245.mat','countyresultdisaster');
        save('chinaresultdisaster245.mat','chinaresultdisaster');
        save('countyreladisaster245.mat','countyreladisaster');
        save('countyreladisasterpower245.mat','countyreladisasterpower');
    elseif ppp == 3
        save('countyresultdisasterinfo370.mat','countyresultdisasterinfo');
        save('countyresultdisaster370.mat','countyresultdisaster');
        save('chinaresultdisaster370.mat','chinaresultdisaster');
        save('countyreladisaster370.mat','countyreladisaster');
        save('countyreladisasterpower370.mat','countyreladisasterpower');
    elseif ppp == 4
        save('countyresultdisasterinfo585.mat','countyresultdisasterinfo');
        save('countyresultdisaster585.mat','countyresultdisaster');
        save('chinaresultdisaster585.mat','chinaresultdisaster');
        save('countyreladisaster585.mat','countyreladisaster');
        save('countyreladisasterpower585.mat','countyreladisasterpower');
    end
end