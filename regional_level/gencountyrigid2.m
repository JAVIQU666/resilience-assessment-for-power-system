...
for ppp=1:4
    ...
    %  索引  0-0.1 0.1-1 1-6 6-15 15-30 30-80 80
    ...
    countyresultrigid = cell(size(monthly_average_load,1),3);
    countyresultrigidinfo = cell(size(monthly_average_load,1),32);
    ...
    for i=1:numcounty
        ...
        for stage = 1:numstage
            ...
            if CtgProbheat ~= 0
                ...
                for j = 1:numScenarios
                    scenarioData = eval([scenarioNames{j}, '(12, indicespv)']);
                    leng = size(scenarioData, 2);
            ...
            if CtgProbtyphoon ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(12, indicestyphoon1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(12, indicestyphoon2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(12, indicestyphoon3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbexwind~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(12, indicesexwind1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(12, indicesexwind2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(12, indicesexwind3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
            ...
            if CtgProbflood ~= 0
                ...
                for k=1:3
                    for j = 1:numScenarios
                        if k == 1
                            scenarioData = eval([scenarioNames{j}, '(12, indicesflood1)']);
                        elseif k == 2
                            scenarioData = eval([scenarioNames{j}, '(12, indicesflood2)']);
                        elseif k == 3
                            scenarioData = eval([scenarioNames{j}, '(12, indicesflood3)']);
                        end
                        leng = size(scenarioData, 2);
                        results(j, :) = num2cell(calculatemulticost(scenarioData, leng, weightsload, days_in_month));
                    end
        ...
        countyresultrigid{i,1} = monthly_average_load(i,1);
        countyresultrigid{i,2} = found_row;
        countyresultrigid{i,3} = result;

        weather_conditions = {'normal', 'heat', 'cold', 'typhoon', 'exwind', 'flood'};
        countyresultrigidinfo{i,1} = monthly_average_load(i,1);
        countyresultrigidinfo{i,2} = found_row;
        for j =1:length(temptotal)
            countyresultrigidinfo{i,j+2} = temptotal(j);
        end

        resultrela.temprela = temprela;
        resultrelapower.temprela = temprelapower;
        countyrelarigid{i,1} = monthly_average_load(i,1);
        countyrelarigid{i,2} = found_row;
        countyrelarigid{i,3} = resultrela;
        countyrelarigidpower{i,1} = monthly_average_load(i,1);
        countyrelarigidpower{i,2} = found_row;
        countyrelarigidpower{i,3} = resultrelapower;
        i;
    end
    chinaresultrigid = [Tgencostnormal;Trelanormal;TDRcostnormal;Trigidcostnormal;Tdistributecostnormal;Tbscostnormal];
    chinaresultrigid = [chinaresultrigid,sum(chinaresultrigid,2)];
    chinaresultrigid(4:end,7) = chinaresultrigid(4:end,6);
    if ppp == 1
        save('countyresultrigidinfo126.mat','countyresultrigidinfo');
        save('countyresultrigid126.mat','countyresultrigid');
        save('chinaresultrigid126.mat','chinaresultrigid');
        save('countyrelarigid126.mat','countyrelarigid'); %without bulk power system
        save('countyrelarigidpower126.mat','countyrelarigidpower');
    elseif ppp == 2
        save('countyresultrigidinfo245.mat','countyresultrigidinfo');
        save('countyresultrigid245.mat','countyresultrigid');
        save('chinaresultrigid245.mat','chinaresultrigid');
        save('countyrelarigid245.mat','countyrelarigid');
        save('countyrelarigidpower245.mat','countyrelarigidpower');
    elseif ppp == 3
        save('countyresultrigidinfo370.mat','countyresultrigidinfo');
        save('countyresultrigid370.mat','countyresultrigid');
        save('chinaresultrigid370.mat','chinaresultrigid');
        save('countyrelarigid370.mat','countyrelarigid');
        save('countyrelarigidpower370.mat','countyrelarigidpower');
    elseif ppp == 4
        save('countyresultrigidinfo585.mat','countyresultrigidinfo');
        save('countyresultrigid585.mat','countyresultrigid');
        save('chinaresultrigid585.mat','chinaresultrigid');
        save('countyrelarigid585.mat','countyrelarigid');
        save('countyrelarigidpower585.mat','countyrelarigidpower');
    end
end
