function [result] = calLshapednormal365withmaingridheat(resultnor,time, totalRela, mpc0, mpcre0, mpcrelcoe0, equipment, mpc_model_load20, Pload, WT, PV, rateload, scen,ProvDisasterheat,pdc,powerinter,firpower)
...
tic
x = 301.5-273.15;
inc1 = (0.22*x*x-4.2*x+116)/60;
x = 305-273.15;
inc2 = (0.22*x*x-4.2*x+116)/60;
x = 308-273.15;
inc3 = (0.22*x*x-4.2*x+116)/60;
...
for stage=1:length(period)
    ...
    if sum(ProvDisasterheat(:,stage))>0
        pl = sum(pl,1)*([inc1,inc2,inc3]*ProvDisasterheat(:,stage))/sum(ProvDisasterheat(:,stage))*(1+powerinter);
    else
