function [result] = calLshapednormal365withmaingridcold(resultnor,time, totalRela, mpc0, mpcre0, mpcrelcoe0, equipment, mpc_model_load20, Pload, WT, PV, rateload, scen,ProvDisastercold,pdc,powerinter,firpower)
...
tic
x = 257.5-273.15;
inc1 = (0.05*x*x-1.5*x+86)/60;
x = 252.5-273.15;
inc2 = (0.05*x*x-1.5*x+86)/60;
x = 249-273.15;
inc3 = (0.05*x*x-1.5*x+86)/60;
...
for stage=1:length(period)
    ...
    wt = wt*wtinc(stage);
    pv = pv*pvinc(stage)*1.4; % eff to p.u.
    ...
    if sum(ProvDisastercold(:,stage))>0
        pl = sum(pl,1)*([inc1,inc2,inc3]*ProvDisastercold(:,stage))/sum(ProvDisastercold(:,stage))*(1+powerinter);
    else
