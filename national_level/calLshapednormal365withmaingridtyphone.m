function [result] = calLshapednormal365withmaingridtyphone(resultnor,time, totalRela, mpc0, mpcre0, mpcrelcoe0, equipment, mpc_model_load20, Pload, WT, PV, rateload, scen,ProvDisaster,pdc,powerinter,firpower)
...
for stage=1:length(period)
    ...
    if sum(ProvDisaster(:,stage))>0
        MpConstraints = [MpConstraints, 0 == CSPcap(:,stage)];
        MpConstraints = [MpConstraints, 0 == UPVcap(:,stage)];
        MpConstraints = [MpConstraints, 0 == DPVcap(:,stage)];
        MpConstraints = [MpConstraints, 0 == ONWTcap(:,stage)];
        MpConstraints = [MpConstraints, 0 == OFFWTcap(stage)];
        pl = sum(pl,1)*(1+0.5*powerinter);
    end
    if stage == 1
        MpConstraints = [MpConstraints, 0 <= BScap(stage) <= 30000, ...
            0 <= NGcap(stage), ...
            0 <= NGCCScap(stage), ...
            maxNGcap <= NGcap(stage) + NGCCScap(stage) <= 1.1*maxNGcap, ...
            0 <= COALcap(stage), ...
            0 <= COALCCScap(stage), ...
            maxCOALcap <= COALcap(stage) + COALCCScap(stage) <= 1.1*maxCOALcap, ...
            maxHycap <= Hydropowercap(stage) <= 2*maxHycap, ...
            0 <= Biopowercap(stage) <= 10000, ...
            0 <= BiopowerCCScap(stage) <= 10000, ...
            0 <= Geothermalcap(stage) <= 10000, ...
            0 <= Nuclearcap(stage) <= 10000, ...
        ...
        MpConstraints = [MpConstraints, BScap(stage-1) <= BScap(stage) <= 30000, ...
            ...
            NGcap(stage) + NGCCScap(stage) <= 1.1*maxNGcap, ...
            COALcap(stage-1) <= COALcap(stage), ...
            COALCCScap(stage-1) <= COALCCScap(stage), ...
            COALcap(stage) + COALCCScap(stage) <= 1.1*maxCOALcap, ...
            Hydropowercap(stage-1) <= Hydropowercap(stage) <= 2*maxHycap, ...
            Biopowercap(stage-1) <= Biopowercap(stage) <= 10000, ...
            BiopowerCCScap(stage-1) <= BiopowerCCScap(stage) <= 10000, ...
            Geothermalcap(stage-1) <= Geothermalcap(stage) <= 10000, ...

            Nuclearcap(stage-1) <= Nuclearcap(stage) <= 10000, ...
    ...
    Constraints = [Constraints, 0 <= (-sum(sum(PBiopowerCCS(:,stage)))*0.35+sum(sum(PBiopower(:,stage)))*0.35 + ...
    ...
    rela(stage) = (1+discountrate)^(2025-period(stage))*discount*pdc*1000*8760/time*sum(sum(plc(:,:,stage)));
    if stage == 1
    ...
    carboncost(stage) = (1+discountrate)^(2025-period(stage))*8760/time*...
        (-sum(sum(PBiopowerCCS(:,stage)))*0.35 + sum(sum(PBiopower(:,stage)))*0.35 + 0.865*0.05*sum(sum(PCOALCCS(:,stage))) + ...
